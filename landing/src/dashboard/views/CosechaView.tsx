...
const eventos = eventosHoy.filter(e => e.tipo === 'cosecha')
...
function parseKg(sub: string): number | null {
  const m = sub.match(/(\d+(?:\.\d+)?)\s*kg/)
  return m ? parseFloat(m[1]) : null
}

const totalKg = eventos.reduce((s, e) => s + (parseKg(e.sub) ?? 0), 0)
...
export function CosechaView() {
  ...
  return (
    <>
      <Topbar
        title="Cosecha"
        badge={`${eventos.length} cosecha${eventos.length !== 1 ? 's' : ''} hoy`}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #0D0F0C', background: 'rgba(13,15,12,0.03)' }}>
                  {['Cosecha', 'Lote', 'Trabajador', 'Hora', 'Fuente', 'Estado', 'Conf.'].map(h => (
