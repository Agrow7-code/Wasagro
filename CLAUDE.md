...
- **D28 — Internal back-office `/admin` (director-only), rescoped to the operable line S2+S3.** S4 (mock→real view migration) and S5 (P&L table + `managementAlertsWorker`) are **DEFERRED** to a future sub-epic — ADR 012's original Clientes/P&L/Billing/Alertas vision still stands as the long-term target, but this PR ships only what makes the founder operate today. **Backend (PR-S2, shipped):** director-only `/api/admin/*` surface. `src/auth/roleGuard.ts` — fail-closed (missing/non-string/non-director `rol` → 403; any thrown error, including from `next()`, → 500 and `next()` is never called again from the catch). `src/agents/admin/router.ts` — `GET /orgs` (actual counts via a PostgREST aggregate embed `fincas(count)`/`usuarios(count)` in ONE round-trip, never N+1, alongside D26 contractual counts), `GET /orgs/:id` (org + fincas[] + usuarios[], phones last-4 masked; billing history omitted — that's S5), `GET /sdr` (phones masked server-side, `status` DB column aliased to `estado` on the wire), `POST /clients` (calls `provisionarCliente()` **directly** — never `createProvisionHandler`, which gates on `REPORTE_SECRET` and must never be reachable from the browser; `roleGuard` is the sole gate). `src/utils/maskPhone.ts` — shared last-4 mask (P5/D31), extracted from the 4-way duplicate in pgBoss.ts/alertaEntrega.ts/provisionarCliente.ts/EventHandler.ts (call sites not yet migrated). Mounted in `src/index.ts` between `/api/auth` and `/api/metricas/*`, deliberately **WITHOUT** `planGuard` — a director's own org billing status must never block admin access to other orgs. **Frontend (PR-S3, not yet shipped):** `/admin` route tree + ClientList/ClientDetail/CreateClientForm/SdrFunnel, with drill-in nav restricted to the 3 already-real views (Sigatoka review, FincaSetup, Billing) — no founder ever reaches a mock view. *Revisar:* PR-S3 delivery; `SUPABASE_ANON_KEY` still unset means `requireFincaAccessAsync` is the sole D31 gate for drill-in (see D31); >50 orgs → server-side pagination. [ADR 012, ADR 020]
...
- **D34 — Umbrales de alerta configurables por org/finca (`umbrales_alerta`).** Reemplaza la lectura de `fincas.config.sigatoka_umbrales` y la env `SIGATOKA_UMBRAL_EE2_LEVE` como fuente de umbrales de alerta. Tabla relacional `umbrales_alerta` (org_id, finca_id NULL=org-default, pest_type, campo, operador, valor, enabled) + `decision_alerta` (estado not_asked/asked/decided/opted_out por org/finca/pest). Precedencia: per-finca > org-default > silent. `enabled=false` = umbral desactivado (nunca magic 101). Centinelas `Infinity`/`-Infinity` en `toUmbralesSeveridad()` para campos no configurados. Resolver puro: `resolveUmbrales()` + `toUmbralesSeveridad()` en `src/pipeline/handlers/umbralesAlerta.ts`. Persistencia: `getUmbralesAlerta`, `upsertUmbralAlerta`, `getDecisionMakersByOrg` (director/admin_org únicamente, D31), `getDecisionAlerta`, `upsertDecisionAlerta` en `supabaseQueries.ts`. Web config: `GET/PUT /api/finca/:id/alertas/config` + `GET/PUT /api/finca/org/:orgId/alertas/config` (auth `requireFincaAccessAsync`/`requireOrgAccessAsync`). Cutover dual-read: EventHandler lee tabla primero; fallback a `parseFincaUmbrales(fincas.config)` solo si `ALERT_THRESHOLDS_DUAL_READ=true` (flag durante migración, se elimina en PR#4). Seed idempotente: org-default desde `UMBRALES_SEVERIDAD_DEFAULT` + per-finca desde `fincas.config` existente (migr. 073, `ON CONFLICT DO NOTHING`). ee2Leve seed `enabled=false` (sin respaldo agronómico, P7). Tablas: migr. 069/070. RLS: REVOKE/GRANT service_role (migr. 074) + `CREATE POLICY` service_role (migr. 075). `requireOrgAccessAsync` retorna `'ok'|'unauthorized'|'forbidden'` (401 vs 403 explícito). `pest_type` normalizado con `canonicalPestType()` antes del catalog lookup; max 50 rules por PUT. Fail-safe explícito en EventHandler: `umbralesFinca = UMBRALES_SEVERIDAD_DEFAULT` garantiza que J/I/M siempre disparan aunque la tabla esté vacía. *Revisar:* PR#4 elimina dual-read + depreca env + deja de escribir `sigatoka_umbrales` en `fincas.config`. [change configurable-alert-thresholds, migr. 068-075]
