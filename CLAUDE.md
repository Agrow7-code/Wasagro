...
- **D23 — Booking de demos con Cal.com self-hosted** (reemplaza link estático Calendly). Webhook `POST /webhook/calcom` (`BOOKING_CREATED`/`CANCELLED`), firma con `CALCOM_WEBHOOK_SECRET`; cancelación NO revierte status automáticamente (P7), notifica al founder. Env `CALCOM_*`, `FOUNDER_PHONE/EMAIL`, `RESEND_API_KEY`. *Revisar:* >50 bookings/semana → batch en pg-boss. *(ADR `009-calcom-over-calendly.md` pendiente de crear — el 009 actual es otro documento.)*
...
- **D29 — Sub-pipeline de muestreo Sigatoka** sobre `documento_tabla` *(aceptada provisionalmente — pendiente del primer formulario en producción).* Detección en dos niveles: fast path en `sp-03c` (enum `muestreo_sigatoka_banano`) y safety net por keywords sobre el OCR genérico. Extractor `extraerMuestreoSigatoka` (`sp-03e`, tier `ocr`) → `SigatokaMuestreoSchema` (Zod). Recálculo determinista de fórmulas (H..M) contra lo escrito a mano; alertas hardcodeadas (J>10%, I>5%, M<9); `requiereValidacion` si confidence < 0.75 o discrepancia. Persiste en `eventos_campo.datos_evento` (JSONB) — preserva integración con métricas (D18), resumen (D14), RAG (D12), cost tracking (D27). `SigatokaHandler.ts`. *Revisar:* (a) primer formulario en prod; (b) 2º formulario agrícola → generalizar el patrón; (c) finca con umbrales distintos → parametrizar; (d) falsos positivos de keywords → subir threshold. [ADR 013]
