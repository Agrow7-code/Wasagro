...
- **Decisión:**
  1. **Detección en DOS niveles para máxima robustez:**
     - **Nivel A (fast path — preferido):** `clasificarTipoImagen` (`sp-03c`, Gemini fast) ahora incluye `'muestreo_sigatoka_banano'` como cuarto valor del enum `TipoImagen`. Detecta visualmente por: título "MUESTREO DE SIGATOKA"/"LOGBAN", matriz P1..P19, columnas H/H+VLE/FUNC/EF PAS, valores con paréntesis "2(3)", sección CERAMIDA/SIBINE, bloque de fórmulas A..M. Cuando devuelve este tipo → EventHandler salta el OCR genérico y va directo al extractor sp-03e. Ahorra ~8-14s + 1 LLM call.
     - **Nivel B (safety net):** Si `sp-03c` devuelve `'documento_tabla'` (no detectó Sigatoka visualmente), se ejecuta OCR genérico y después `detectarFormularioSigatoka(ocr.texto_completo_visible)` corre keyword check sobre marcadores `SIGATOKA`, `H+VLE`, `EF PAS`, `EF ACT`, `FUNC`, `EE2`, `EE3`, `CERAMIDA`, `SIBINE` (threshold 3+). Cubre el caso donde el clasificador visual falla pero el OCR sí transcribe los headers. **No requiere LLM adicional** — reutiliza output del OCR ya hecho.
